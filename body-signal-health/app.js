...
function demoData(){const out=[],base=67.0,totalDays=400;for(let i=totalDays-1;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const train=i%3===1;const hard=i%9===1;const trainedYesterday=(i+1)%3===1;const longTrend=(totalDays-1-i)*.0012,recentTrend=i<35?(35-i)*.015:0,seasonal=Math.sin(i/18)*.16,recoveryWater=trainedYesterday?.22:0;out.push({id:'d'+i,date:iso(d),weight:+(base+longTrend+recentTrend+seasonal+recoveryWater+Math.sin(i*1.7)*.08).toFixed(1),waist:+(32.8+Math.sin(i/23)*.1+Math.sin(i)*.04).toFixed(1),sleep_hours:+(6.6+(i%4)*.3).toFixed(1),sleep_quality:3+(i%3===0?1:0),soreness:trainedYesterday?(hard?7:4):(train?3:1),resting_hr:61+(hard?4:0),bloating:i%8===0?2:0,did_workout:train,workout_type:hard?'strength':'cardio',duration_min:train?35:0,rpe:train?(hard?8:4):null,z1:train?6:0,z2:train?(hard?8:25):0,z3:train?(hard?10:3):0,z4:hard?7:0,z5:hard?2:0,note:i%8===0?'กินเค็มเมื่อคืน':null})}return out}
...
function workoutWeightContext(latestWeight){if(!latestWeight)return null;const recent=entries.filter(e=>e.did_workout&&e.date>=dateShift(latestWeight.date,-2)&&e.date<latestWeight.date),priorLoads=entries.filter(e=>e.did_workout&&e.date<latestWeight.date&&load(e)>0).map(load),usualLoad=median(priorLoads),hard=recent.some(e=>e.workout_type==='strength'||num(e.rpe)>=7||(load(e)>0&&usualLoad!=null&&load(e)>usualLoad*1.2)),pairs=[];entries.forEach(e=>{if(!e.did_workout||e.date>=latestWeight.date)return;const next=entries.find(x=>x.date===dateShift(e.date,1)&&num(x.weight)!=null),baseline=mean(inWindow(entries,dateShift(e.date,-1),7).map(x=>num(x.weight)).filter(Number.isFinite));if(next&&baseline!=null)pairs.push(num(next.weight)-baseline)});return {recentCount:recent.length,hard,pairCount:pairs.length,averageDelta:mean(pairs)}}
function workoutWeightExplanation(context){if(!context?.recentCount)return '';let text=`มีการออกกำลังกาย${context.hard?'ค่อนข้างหนัก':''}ในช่วง 2 วันที่ผ่านมา น้ำหนักที่เพิ่มอาจเกี่ยวกับน้ำและการฟื้นตัวของกล้ามเนื้อ ยังไม่ควรสรุปว่าเป็นไขมัน`;if(context.pairCount>=4&&context.averageDelta>.1)text+=` จากข้อมูลของคุณ ${context.pairCount} ครั้งที่ผ่านมา น้ำหนักวันถัดไปมักสูงกว่าแนวโน้มเดิมเฉลี่ย ${context.averageDelta.toFixed(1)} กก.`;return text}
...
function renderSimpleStatus(){
  ...
  const waistEntries=entries.filter(e=>num(e.waist)!=null),latestWaist=waistEntries.at(-1),waists=recent.map(e=>num(e.waist)).filter(Number.isFinite),oldWaists=previous.map(e=>num(e.waist)).filter(Number.isFinite),referenceWaists=inWindow(entries,anchor,28,7).map(e=>num(e.waist)).filter(Number.isFinite),olderReferenceWaists=inWindow(entries,anchor,28,14).map(e=>num(e.waist)).filter(Number.isFinite),waistNow=median(waists),waistOld=median(oldWaists),waistBase=median(referenceWaists),olderWaistBase=median(olderReferenceWaists),waistRise=waistNow!=null&&waistBase!=null?waistNow-waistBase:null,priorRise=waistOld!=null&&olderWaistBase!=null?waistOld-olderWaistBase:null,confirmedRise=waistRise>=1&&priorRise>=1,ratio=waistNow!=null&&num(profile.height_cm)?waistNow*2.54/num(profile.height_cm):null,exerciseContext=workoutWeightContext(latestWeight),exerciseText=workoutWeightExplanation(exerciseContext);
  $('weightStatusValue').textContent=latestWeight?`${num(latestWeight.weight).toFixed(1)} kg`:'—';$('weightTrendValue').textContent=avg!=null?`เฉลี่ย 7 วัน · ${avg.toFixed(1)} kg`:'เฉลี่ย 7 วัน · —';let wTitle='ยังบอกไม่ได้',wText='บันทึกน้ำหนักอย่างน้อยหนึ่งครั้งเพื่อเริ่มดูแนวโน้ม',wLevel='';if(latestWeight){if(!weights.length){wTitle='ยังไม่มีน้ำหนักใน 7 วันล่าสุด';wText=`ชั่งครั้งล่าสุดเมื่อ ${fmt(latestWeight.date)}`}else if(diff==null){wTitle='มีข้อมูลน้ำหนักแล้ว';wText='ยังไม่มีข้อมูลจากสัปดาห์ก่อนให้ดูว่าเปลี่ยนไปหรือไม่'}else if(Math.abs(diff)<.2){wTitle=diff>0?'น้ำหนักขยับขึ้นเล็กน้อย':diff<0?'น้ำหนักขยับลงเล็กน้อย':waistRise!=null&&waistRise<.5?'น้ำหนักและรอบเอวยังคงเดิม':'น้ำหนักค่อนข้างคงที่';wText=diff===0?'น้ำหนักเฉลี่ย 7 วันเท่าเดิมเมื่อเทียบกับสัปดาห์ก่อน':`น้ำหนักเฉลี่ย 7 วัน${diff>0?'เพิ่มขึ้น':'ลดลง'}จากสัปดาห์ก่อนเพียง ${Math.abs(diff).toFixed(1)} กก.`;if(waistRise!=null&&waistRise<.5)wText+=` รอบเอวยังไม่มีแนวโน้มขยายชัดเจน ${diff>0?(exerciseText||'น้ำหนักที่เพิ่มอาจมาจากน้ำ อาหาร หรือการเปลี่ยนแปลงของมวลร่างกาย'):''}`.trimEnd()}else{wText=`น้ำหนักเฉลี่ย 7 วัน${diff>0?'เพิ่มขึ้น':'ลดลง'}จากสัปดาห์ก่อน ${Math.abs(diff).toFixed(1)} กก.`;if(diff>0&&waistRise!=null&&waistRise<.5){wTitle='น้ำหนักเพิ่ม แต่รอบเอวยังไม่ขยายชัดเจน';wText+=` รอบเอวยังไม่มีแนวโน้มขยายชัดเจน ${exerciseText||'น้ำหนักที่เพิ่มอาจมาจากน้ำ อาหาร หรือการเปลี่ยนแปลงของมวลร่างกาย'}`}else if(diff>0&&waistRise!=null&&waistRise>=.5){wTitle=confirmedRise?'น้ำหนักและรอบเอวเพิ่มต่อเนื่อง':'น้ำหนักเพิ่มและเอวเริ่มขยับ';wLevel=confirmedRise?'alert':'caution';wText+=confirmedRise?' และรอบเอวเพิ่มต่อเนื่อง ควรเริ่มทบทวนอาหารและกิจกรรมช่วงที่ผ่านมา':' พร้อมกับรอบเอวที่เริ่มเพิ่ม ควรวัดซ้ำเพื่อยืนยัน'}else if(diff>0){wTitle='น้ำหนักเพิ่ม แต่ยังไม่มีรอบเอวให้เทียบ';wText+=exerciseText?` ${exerciseText}`:' ลองบันทึกรอบเอวเพื่อดูว่ารูปร่างเปลี่ยนหรือเป็นเพียงน้ำหนักที่แกว่ง'}else if(diff<0){wTitle='น้ำหนักลดลงจากสัปดาห์ก่อน';if(waistRise!=null&&waistRise<.5)wText+=' และรอบเอวยังไม่มีแนวโน้มขยายชัดเจน'}else{wTitle='น้ำหนักค่อนข้างคงที่'}}}$('weightStatusTitle').textContent=wTitle;$('weightStatusText').textContent=wText;setStatusCard('weightStatusCard',wLevel);
  $('waistStatusValue').textContent=latestWaist?`${num(latestWaist.waist).toFixed(1)} นิ้ว`:'—';$('waistTrendValue').textContent=waistNow!=null?`ค่ากลาง 7 วัน · ${waistNow.toFixed(1)} นิ้ว`:'ค่ากลาง 7 วัน · —';let waTitle='ยังบอกไม่ได้',waText='บันทึกรอบเอวเพื่อดูว่าช่วงเอวกำลังขยายหรือไม่',waLevel='';if(latestWaist){if(waistNow==null){waTitle='ยังไม่มีรอบเอวใน 7 วันล่าสุด';waText=`วัดครั้งล่าสุดเมื่อ ${fmt(latestWaist.date)}`}else if(waistRise==null){waTitle='มีข้อมูลรอบเอวแล้ว';waText='ยังไม่มีข้อมูลจากช่วงก่อนหน้าให้ดูว่ารอบเอวเปลี่ยนไปหรือไม่'}else if(confirmedRise){waTitle='รอบเอวเพิ่มต่อเนื่อง ควรระวัง';waLevel='alert';waText=`ค่ากลางรอบเอว 7 วันเพิ่มขึ้นจากระดับปกติประมาณ ${waistRise.toFixed(1)} นิ้ว และพบว่าเพิ่มต่อเนื่องสองสัปดาห์`}else if(waistRise>=1){waTitle='รอบเอวเพิ่มขึ้นชัดเจน';waLevel='caution';waText=`ค่ากลางรอบเอว 7 วันเพิ่มขึ้นจากช่วงก่อนประมาณ ${waistRise.toFixed(1)} นิ้ว ควรวัดยืนยันอีกครั้งในสัปดาห์หน้า`}else if(waistRise>=.5){waTitle='รอบเอวเริ่มเพิ่มขึ้น';waLevel='caution';waText=`ค่ากลางรอบเอว 7 วันเพิ่มขึ้นจากช่วงก่อนประมาณ ${waistRise.toFixed(1)} นิ้ว ควรวัดซ้ำในเวลาและตำแหน่งเดิม${exerciseContext?.recentCount?' เพราะมีการออกกำลังกายใน 2 วันก่อนวัด จึงอาจเป็นการเปลี่ยนแปลงชั่วคราว':''}`}else{waTitle='รอบเอวยังคงเดิม';waText=waistRise===0?'ค่ากลางรอบเอว 7 วันเท่าเดิมเมื่อเทียบกับช่วงก่อน':`ค่ากลางรอบเอว 7 วัน${waistRise>0?'เพิ่มขึ้น':'ลดลง'}จากช่วงก่อนเพียง ${Math.abs(waistRise).toFixed(1)} นิ้ว`;if(waistRise>0&&exerciseContext?.recentCount)waText+=' มีการออกกำลังกายใน 2 วันก่อนวัด จึงควรติดตามต่อก่อนสรุปว่าเอวขยาย'}if(waists.length===1)waText+=' แต่ตอนนี้มีข้อมูลเพียงครั้งเดียว จึงยังยืนยันแนวโน้มไม่ได้';if(ratio!=null){if(ratio>=.6){waLevel='alert';waText+=' เมื่อเทียบกับส่วนสูง รอบเอวอยู่ในระดับที่ควรใส่ใจและไม่ควรเพิ่มขึ้นอีก'}else if(ratio>=.5){if(waLevel!=='alert')waLevel='caution';waText+=' ตอนนี้รอบเอวเท่ากับครึ่งหนึ่งของส่วนสูงแล้ว ควรรักษาไม่ให้เพิ่มไปกว่านี้'}else{waText+=' เมื่อเทียบกับส่วนสูง รอบเอวยังต่ำกว่าระดับที่ควรเริ่มระวัง'}}}$('waistStatusTitle').textContent=waTitle;$('waistStatusText').textContent=waText;setStatusCard('waistStatusCard',waLevel);
...
เกณฑ์นี้ใช้สังเกตการเปลี่ยนภาระ ไม่ใช่การทำนายการบาดเจ็บ และควรดูการนอน อาการปวด ชีพจรพัก และความรู้สึกจริงร่วมด้วย`],
  simpleWeight:['น้ำหนักคำนวณอย่างไร',`ตัวเลขใหญ่คือน้ำหนักครั้งล่าสุด ส่วนตัวเลขเล็กด้านล่างคือน้ำหนักเฉลี่ยจากข้อมูลที่บันทึกใน 7 วันล่าสุด

ระบบไม่ตัดสินว่าอ้วนจากน้ำหนักที่เพิ่มเพียงอย่างเดียว แต่คำนวณร่วมกับรอบเอว
...
การออกกำลังกายใช้เป็นข้อมูลประกอบเมื่อมีการออกใน 1–2 วันก่อนชั่ง ระบบดูประเภทการฝึก RPE และ Edwards Load เพื่อบอกว่ามีการฝึกค่อนข้างหนักหรือไม่ หากมีคู่ข้อมูลอย่างน้อย 4 ครั้ง ระบบจะเปรียบเทียบน้ำหนักวันถัดไปกับแนวโน้ม 7 วันก่อนออกกำลังกายของคุณเอง

ผลนี้แสดงความสัมพันธ์เท่านั้น ระบบไม่หักน้ำหนักออกและไม่สรุปว่าการออกกำลังกายเป็นสาเหตุแน่นอน

ระบบคำนวณจากข้อมูลเท่าที่บันทึกและไม่เติมค่าของวันที่ขาดหาย`],
  simpleWaist:['รอบเอวคำนวณอย่างไร',`การ์ดนี้แยก “แนวโน้มส่วนตัว” ออกจาก “ระดับสุขภาพ” เพื่อไม่ให้รอบเอวที่คงที่ถูกแปลว่ากำลังเพิ่ม

ค่าที่แสดงตัวใหญ่ = รอบเอวครั้งล่าสุด
ค่าตัวเล็กด้านล่าง = มัธยฐานหรือค่ากลางของรอบเอวที่บันทึกใน 7 วันล่าสุด
แนวโน้มปัจจุบัน = มัธยฐานของค่าที่บันทึกใน 7 วันล่าสุด
ฐานย้อนหลัง = มัธยฐานของค่าที่บันทึกในวันที่ 8–35 ย้อนหลัง
ส่วนต่าง = แนวโน้มปัจจุบัน − ฐานย้อนหลัง

มัธยฐานคือการเรียงค่าจากน้อยไปมากแล้วเลือกค่ากลาง ถ้ามีจำนวนคู่จะเฉลี่ยสองค่ากลาง วิธีนี้ช่วยลดผลจากวันที่ท้องอืดหรือวัดคลาดเคลื่อนหนึ่งครั้ง

• ต่ำกว่า +0.5 นิ้ว = คงที่
• +0.5 ถึงต่ำกว่า +1 นิ้ว = เริ่มจับตา
• ตั้งแต่ +1 นิ้ว = วัดยืนยัน และจะเตือนว่าเพิ่มต่อเนื่องเมื่อพบสองสัปดาห์ติดกัน

ระบบแสดงจำนวนครั้งที่ใช้คำนวณเสมอ ถ้ามีเพียงครั้งเดียวจะยังคำนวณจากข้อมูลนั้น แต่บอกว่าผลอาจแกว่งได้

ระดับสุขภาพคำนวณแยกด้วยสูตร: รอบเอว×2.54 ÷ ส่วนสูงเซนติเมตร
• ต่ำกว่า 0.50 = รอบเอวยังน้อยกว่าครึ่งหนึ่งของส่วนสูง
• 0.50–0.59 = ควรควบคุมไม่ให้เพิ่ม
• ตั้งแต่ 0.60 = ความเสี่ยงสูงขึ้น

เกณฑ์การเพิ่ม 0.5 และ 1 นิ้วใช้ติดตามการเปลี่ยนแปลงส่วนตัว ไม่ใช่เกณฑ์วินิจฉัยโรค`],
...
เส้นแนวโน้ม ณ วันที่ใด = ผลรวมน้ำหนักที่มีตั้งแต่วันนั้นย้อนหลังรวม 7 วันปฏิทิน ÷ จำนวนครั้งที่ชั่งในช่วงนั้น
...
เส้นแนวโน้ม ณ วันที่ใด = ค่ามัธยฐานของรอบเอวที่มีตั้งแต่วันนั้นย้อนหลังรวม 7 วันปฏิทิน
